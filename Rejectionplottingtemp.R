...
Calibration_plots_fbw <- function(datatype, rho,
  ...
  for (it in 1:reps) {
    ...
    for (ss in sampleS) {
      for (perm in Permutations) {
        Pval1 <- dhsic.test(X[1:ss,],Y[1:ss,],kernel = "gaussian.fixed", bandwidth = BW[1] , B=perm)$p.value
        P1 <- data.frame("Pval"=Pval1,"SampleSize"=ss,"Permutations"=perm,"Estimator"=as.character(BW[1]))
        Pval2 <- dhsic.test(X[1:ss,],Y[1:ss,],kernel = "gaussian.fixed", bandwidth = BW[2] , B=perm)$p.value
        P2 <- data.frame("Pval"=Pval1,"SampleSize"=ss,"Permutations"=perm,"Estimator"=as.character(BW[2]))
        Pval3 <- dhsic.test(X[1:ss,],Y[1:ss,],kernel = "gaussian.fixed", bandwidth = BW[3] , B=perm)$p.value
        P3 <- data.frame("Pval"=Pval1,"SampleSize"=ss,"Permutations"=perm,"Estimator"=as.character(BW[3]))
        Pval4 <- dhsic.test(X[1:ss,],Y[1:ss,],kernel = "gaussian.fixed", bandwidth = BW[4] , B=perm)$p.value
